$(document).on 'turbolinks:load', ->
  $('.new_member').find('#member_email, #member_name').keypress (e) ->
    if e.which == 13 && valid_email($('.new_member').find( "#member_email" ).val()) && $('.new_member').find( "#member_name" ).val() != ""
      $('.new_member').submit()

  $('.new_member').find('#member_email, #member_name').bind 'blur', ->
    if valid_email($('.new_member').find( "#member_email" ).val()) && $('.new_member').find( "#member_name" ).val() != ""
      $('.new_member').submit()
  ...
  $('.new_member').on 'submit', (e) ->
    $.ajax e.target.action,
        ...
        success: (data, text, jqXHR) ->
          ...
          $('.new_member').find('#member_name, #member_email').val("")
          $('.new_member').find('#member_name').focus()
          Materialize.toast('Membro adicionado', 4000, 'green')
        error: (jqXHR, textStatus, errorThrown) ->
          Materialize.toast('Problema na hora de incluir membro', 4000, 'red')
    return false

  $('.update_member').find('#member_email, #member_name').bind 'blur', (e) ->
    member = $(e.target.closest('.update_member'))
    if valid_email(member.find( "#member_email" ).val()) && member.find( "#member_name" ).val() != ""
      member.submit()
